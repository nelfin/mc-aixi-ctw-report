...
\centering
 \begin{tikzpicture}
    \begin{axis}[xlabel=Cycle,ylabel=Average reward,title={Coinflip $\theta=1.0$}]
...
% \end{figure}
These results show that the in a steep curve, the agent learns after about 500 cycles that listening is a good idea (as a reward of 99 is better than picking a rarndom door resulting in an average reward of 55). From this point onwards the agent seems to begin to notice the correlation between the percept provided by listen and the reward given by a door, and there is another small spike. The agent has likely had insufficient time to learn, so the reward has not converged, and is still far from the theoretical optimum of 107 average reward.
...
\end{center}
The CT depth was chosen in order to be slightly more than enough to play optimally. The horizon was chosen both to enable optimal strategy. While this could possibly be set to 3 to make sure there is no unfair bias, this is reasonable because the maximum iterations per environment cycle are 2.
